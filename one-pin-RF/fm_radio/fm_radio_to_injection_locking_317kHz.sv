...
module top (
  ...
  always @(negedge CLK_168_MHZ) r42m0 <= ~r42m0;
  ...
  SB_PLL40_2F_PAD #(
    ...
    //.DIVF(7'd8), // less MHz  // feedback multiplier 0..63 => REF_CLK * 1..64
  ...
  //reg [1:0] r42m0 = 2'd0;
  //always @(posedge CLK_168_MHZ) r42m0 <= {r42m0[0], ~r42m0[1]};
  ...
  //reg [CH1_MSB:0] dds_ch1;
  //reg [CH1_MSB:0] dds_ch1_90 = 45'd8796093022208;
  reg [CH1_MSB:0] dds_ch1_90;
  reg [CH1_MSB:0] dds_ch1 = 49'd140737488355328;
  //always @(posedge CLK_21_MHZ) dds_ch1 <= dds_ch1 + 45'd14911471980505;
  //always @(posedge CLK_21_MHZ) dds_ch1_90 <= dds_ch1_90 + 45'd14911471980505;
  
  // 88.1MHz
  //always @(posedge CLK_21_MHZ) dds_ch1 <= dds_ch1 + 49'd109909276620351;
  //always @(posedge CLK_21_MHZ) dds_ch1_90 <= dds_ch1_90 + 49'd109909276620351;
 
  // 88.097160MHz
  //always @(posedge CLK_21_MHZ) dds_ch1 <= dds_ch1 + 49'd109833144340936;
  //always @(posedge CLK_21_MHZ) dds_ch1_90 <= dds_ch1_90 + 49'd109833144340936;

  // 88.124555MHz
  //always @(posedge CLK_21_MHZ) dds_ch1 <= dds_ch1 + 49'd110567525958745;
  //always @(posedge CLK_21_MHZ) dds_ch1_90 <= dds_ch1_90 + 49'd110567525958745;

  always @(posedge CLK_21_MHZ) dds_ch1 <= dds_ch1 + 49'd539572196669882;
  always @(posedge CLK_21_MHZ) dds_ch1_90 <= dds_ch1_90 + 49'd539572196669882;
  ...
  */
  ...
  localparam SD_DAC_MSB = 8;
  localparam SD_DAC_BITS_MINUS_ONE = 8'd0;
  reg [SD_DAC_MSB:0] sd_dac;

  always @(posedge CLK_21_MHZ) begin

    samples <= decimator[DECIMATOR_MSB] ? 32'd0 : (
      samples
      - { 27'd0, sum }
      + { 27'd0, sum_qq }
    );
  ...
  always @(posedge CLK_21_MHZ) begin
    sd_dac <= decimator[DECIMATOR_MSB] ?
      (sd_dac + samples_bb[SD_DAC_MSB:0]
        + { SD_DAC_BITS_MINUS_ONE, ~sd_dac[SD_DAC_MSB] }) : sd_dac;
  end
  ...
  // 328.126kHz vs. resonant 317.5kHz

  // 4 phases
  /*
  reg [31:0] r317k_0 = 33'b0000_0000_0000_0000_0000_0000_0000_0000;
  reg [31:0] r317k_1 = 33'b1111_1111_1111_1111_1111_1111_1111_1111;
  reg [31:0] r317k_2 = 33'b1111_1111_1111_1111_0000_0000_0000_0000;
  reg [31:0] r317k_3 = 33'b0000_0000_0000_0000_1111_1111_1111_1111;
  always @(posedge CLK_21_MHZ) r317k_0 <= {r317k_0[30:0], ~r317k_0[31]};
  always @(posedge CLK_21_MHZ) r317k_1 <= {r317k_1[30:0], ~r317k_1[31]};
  always @(posedge CLK_21_MHZ) r317k_2 <= {r317k_2[30:0], ~r317k_2[31]};
  always @(posedge CLK_21_MHZ) r317k_3 <= {r317k_3[30:0], ~r317k_3[31]};

  assign PIN2 = ~sd_dac[SD_DAC_MSB] ? r317k_0[0] : r317k_1[0];
  */

  reg [1:0] r317k_0 = 2'b00; //_0000; //_0000_0000_0000_0000_0000_0000;
//  reg [7:0] r317k_1 = 8'b1111_1111_1111_1111_1111_1111_1111_1111;
//  reg [7:0] r317k_2 = 8'b1111_1111_1111_1111_0000_0000_0000_0000;
//  reg [7:0] r317k_3 = 8'b0000_0000_0000_0000_1111_1111_1111_1111;
  always @(posedge CLK_21_MHZ) r317k_0 <= {r317k_0[0], ~r317k_0[1]};
  //always @(posedge CLK_168_MHZ) r317k_0 <= {r317k_0[0], ~r317k_0[1]};
//  always @(posedge CLK_21_MHZ) r317k_1 <= {r317k_1[6:0], ~r317k_1[7]};
//  always @(posedge CLK_21_MHZ) r317k_2 <= {r317k_2[6:0], ~r317k_2[7]};
//  always @(posedge CLK_21_MHZ) r317k_3 <= {r317k_3[6:0], ~r317k_3[7]};

  // 1.2 MHz
  // assign PIN2 = ~sd_dac[SD_DAC_MSB] ? r317k_0[0] : r317k_1[0];
  assign PIN2 = r317k_0[0];// : r317k_1[0];
  //assign PIN2 = CLK_21_MHZ;// : r317k_1[0];
  //assign PIN2 = ~sd_dac[SD_DAC_MSB] ? r317k_0[0] : ~r317k_0[0];
  //assign PIN2 = ~sd_dac[SD_DAC_MSB] ? CLK : ~CLK;
  //assign PIN2 = CLK;
  //assign PIN2 = CLK_168_MHZ;

  // assign PIN2 = ~sd_dac[SD_DAC_MSB];
  
  //assign PIN2 = 1'b0;
  //assign PIN2 = r317k_0[0];
